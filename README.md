...
- ``gfan4.maximal``: reduced Grobner bases, with generators of the 
  symmetric group `S_4`, for the open cones of the
...
- ``cones.gfp4``: cones of ``GF_4|P``, one for each orbit,
  listed in ``output/gfan4.gfp``

Script:

- ``parsegfangrobner.py``: this Python 2.7 code takes in
  ``rays.gfp4`` and ``cones.gfp4``, 
...
Take this number, divide by 2 and subtract the number of maximal cones
(since each orbit is displayed twice each time, and the maximal cones
are display 4 times).
...
Finally, run ``parsegfangrobner.py`` with input names ``input/rays.gfp4``,
``input/cones.gfp4``. 

This returns 1013 - the number of full-dimensional polytrope cones
of ``GF_4|P``.
